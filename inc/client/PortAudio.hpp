                        ...
                        void *userData)
{
    ...
    if (userStruct->encoder != NULL || userStruct->udp != NULL) {
        throw std::exception();
    }
    if (audioData->empty()) {
                          ...
                          void *userData)
{
    ...
    UdpClient *client = (UdpClient *)(userStruct->udp);
    client->sendStart(audio);
    return (paContinue);
}
...
private:
    ...
    bool createRecordStream(OpusEncoder *, UdpClient *);
...
public:
    ...
    PortAudio(void);
    ~PortAudio(void);
    bool createStreams(OpusEncoder *, OpusDecoder *, UdpClient *);
