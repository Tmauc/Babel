...
bool PortAudio::createRecordStream(OpusEncoder *encoder)
{
...
bool PortAudio::createStreams(OpusEncoder *encoder, OpusDecoder *decoder)
{
    if (!this->createPlayStream(decoder)) {
        return false;
    }
    if (!this->createRecordStream(encoder)) {
        return false;
    }
    return true;
}
